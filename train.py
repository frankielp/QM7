...
def train_ml(model,datatype):
    # Load data
    print("Load data")
    datadir = "data/qm7.mat"
    if datatype=="eigenspectrum":
    ...
    for val_fold in tqdm(range(n_folds)):
        X_train, y_train, X_val, y_val = train_val_split(data, val_fold, datatype)
        if model== "linear_regression":
            loss = linear_regression(X_train, y_train, X_val, y_val)
        elif model== "gaussian_process_regression":
            loss = gaussian_process_regression(X_train, y_train, X_val, y_val)
